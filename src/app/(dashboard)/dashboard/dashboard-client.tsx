 ...
 * 의존성   : ./defect-sheet-parser(parseDefectSheet), @/lib/upload-config(BATCH_SIZE), xlsx
...
// 엑셀 파싱 순수 함수는 defect-sheet-parser.ts로 분리 (동작 동일 추출)
import { parseDefectSheet } from './defect-sheet-parser'
...
function buildTemplateWorkbook(site: SiteOption | null, rows: PlantingRow[]) {
  ...
  if (rows.length > 0) {
    ...
    // 샘플 2행 (협력사·계절·단가 채워서 업로드 즉시 성공 가능)
    aoa.push([
      '2024-03-15', siteCode || 'SITE-001', siteName || '샘플현장',
      occupancy || '2024-06-30', startDate || '2024-03-15', '샘플조경',
      '느티나무', 4.5, 3.0, '', 15, 100, 8, region || '경기도',
      180000, '봄', 'H4.5×W3.0×R15', '저위험', '유지 관리', '', 1440000,
    ])
    aoa.push([
      '2024-01-10', siteCode || 'SITE-001', siteName || '샘플현장',
      occupancy || '2024-06-30', startDate || '2024-01-10', '샘플조경',
      '소나무', 3.0, 1.5, 8, '', 50, 15, region || '경기도',
      120000, '겨울', 'H3.0×W1.5×B8', '고위험', '즉시 교체 검토', '전문업체 점검', 1800000,
    ])
  }
  ...
  const guideAoa = [
    ['컬럼명', '필수', '형식/예시', '설명'],
    ['날짜', '선택', '2024-03-15', 'YYYY-MM-DD 형식 권장'],
    ['현장코드', '필수*', 'ABC-001', '없으면 현장명으로 자동 생성 (승인 대기 상태로 등록)'],
    ['현장명', '필수*', '만촌현대', '현장코드 없을 때 필수'],
    ['준공일', '선택', '2024-06-30', 'YYYY-MM-DD'],
    ['식재시기', '선택', '2024-03-15', '계절(수식) 미입력 시 이 날짜로 식재 계절 자동계산. YYYY-MM-DD'],
    ['협력사', '권장', '주현조경', '없으면 미지정으로 등록. 협력사별 분석에 영향'],
    ['수종명', '필수', '느티나무', '한국 수목명 정확히 입력'],
    ['수고 H(m)', '권장', '4.5', '미터 단위 숫자'],
    ['수관폭 W(m)', '권장', '3.0', '미터 단위 숫자'],
    ['흉고직경 B(cm)', '권장', '12', 'cm 단위 숫자. 침엽수(소나무·전나무 등)에 주로 사용'],
    ['근원직경 R(cm)', '권장', '15', 'cm 단위 숫자. 활엽수(느티나무·왕벚나무 등)에 주로 사용'],
    ['수량', '필수', '100', '식재 수량 (정수)'],
    ...
    ['권장조치', '선택', '유지 관리', ''],
    ['세부조치', '선택', '관수 2회/주', ''],
    ['예상 예비비(₩)', '선택', '1440000', '없으면 단가×하자수량으로 자동계산'],
    [],
    ['※ 필수*: 현장코드 또는 현장명 중 하나는 반드시 입력', '', '', ''],
    ['※ 현장코드가 DB에 없으면 신규 현장으로 자동 등록 (설정 > 현장 승인에서 승인 필요)', '', '', ''],
    ['※ 권장★ 컬럼은 분석 정확도에 직접 영향 — 가능한 모든 행에 입력 권장', '', '', ''],
  ]
  const wsGuide = XLSX.utils.aoa_to_sheet(guideAoa)
  wsGuide['!cols'] = [{ wch: 20 }, { wch: 8 }, { wch: 22 }, { wch: 60 }]
