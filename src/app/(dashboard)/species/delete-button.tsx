...
import { deleteSpecies } from '@/app/actions/species'

export function SpeciesDeleteButton({ id }: { id: string }) {
    ...
    return (
      <div className="flex items-center gap-1">
        ...
        <Button size="sm" variant="destructive" className="h-7 px-2 text-xs" disabled={isPending}
          onClick={() => startTransition(async () => { await deleteSpecies(id); setConfirm(false) })}>
          {isPending ? '...' : '확인'}
        </Button>
